...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["uid"]) && isset($_POST["puerta"])) {
        ...
        if ($idEmpleadoAcceso !== null) {
            ...
            $horaAcceso = date("H:i:s"); // Ahora usará la hora de Tijuana
            $fechaAcceso = date("Y-m-d"); // La fecha también se basará en la zona horaria
            $codigoArea = ($puerta == 1) ? 502 : 201; // Código de área INVERTIDO
            // $codigoArea = ($puerta == 1) ? 201 : 202; // Código de área ORIGINAL
            ...
            $idAcceso = $pdo->lastInsertId(); // Obtiene el ID del último acceso insertado
